...
class Strategy:
            ...
            if row.Ticker in df.values:
                ...
                if df_row.iloc[0]['Close'] < df_row.iloc[0]['ema21']:
                    ...
                    dict1.update({
                        ...
                        'Exit_Date': d,
                        'Exit_Price': df_row.iloc[0]['Close'],
                        'Gain': df_row.iloc[0]['Close'] / row.Entry_Price - 1
                    })
            ...
            if row.Ticker in df.values:
                ...
                if df_row.iloc[0]['Close'] > df_row.iloc[0]['ema21']:
                    ...
                    dict1.update({
                        ...
                        'Exit_Date': d,
                        'Exit_Price': df_row.iloc[0]['Close'],
                        'Gain': row.Entry_Price / df_row.iloc[0]['Close'] - 1
                    })
                    rows_list.append(dict1)
        print("SL triggered for ", rows_list.__len__(), " symbols")
        if len(self.closed_pos.index) == 0:
    ...
    def evaluate(self):
        ranked_files = glob.glob(self.rank_data_location + "*_2022*.csv")
        ranked_files.sort()
        i = 0
        max_positions = 15
